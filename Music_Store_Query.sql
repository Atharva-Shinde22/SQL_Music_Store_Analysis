...
SELECT name AS Name, milliseconds
FROM track
WHERE milliseconds > (
...
WITH best_selling_artist AS (
							 SELECT ar.artist_id, ar.artist_name, SUM(il.unit_price * il.price_quantity) AS total_sales
                             FROM invoiceline il
                             JOIN track t ON t.track_id = il.track_id
...
SELECT c.customer_id, c.first_name, c.last_name, bsa.artist_name, SUM(il.unit_ptice * il.quantity) AS amount_spent
FROM invoice i
...
(
  SELECT COUNT(il.quantity) AS Purchases, c.country AS Country, g.name, g.genre_id,
...
WITH RECURSIVE
       sales_per_country AS(
			  SELECT COUNT(*) AS purchase_per_genre, c.country AS Country, g.name AS Genre, g.genre
              FROM invoice_line il
			  JOIN invoice i ON i.invoice_id = il.invoice_id
              JOIN customer c ON c.customer_id = i.customer_id
			  JOIN track t ON t.track_id = il.track_id
...
JOIN max_genre_per_country mg ON sc.country = mg.country
WHERE sc.purchase_per_genre = mg.max_genre_number;
...
WITH customer_with_country AS (
			SELECT c.csutomer_id, first_name, last_name, billing_country, SUM(total) AS total_spending,
...
ON cc.total_spending = ms.billing_country
